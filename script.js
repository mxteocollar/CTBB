...
function buyClickMultiplier(quantity) {
  ...
  if (bought > 0) {
    ...
    totalUpgrades += bought;
    updateUI();
    checkAchievements();
    saveGame(); // Guarda después de cada compra
  }
}
...
startBtn.addEventListener('click', () => {
  ...
  if (score === 0 && clickMultiplierLevel === 0 && autoClickerLevel === 0) {
    score = 0;
...
function getCooldownRemaining() {
  ...
  return Math.max(0, CHEST_COOLDOWN_SECONDS - elapsedSeconds);
}
...
function openChest_Modal() {
  ...
  setTimeout(() => {
    ...
    if (roll < 70) {
      const rewardPoints = Math.floor(Math.random() * 81) + 20; // 20–100
      score += rewardPoints;
...
let ballStore = [
  { id: 'white', color: 'white', price: 0, bought: true },
  { id: 'red',   color: 'red', price: 500, bought: false },
  { id: 'blue',  color: 'blue', price: 1000, bought: false },
  { id: 'green', color: 'green', price: 1500, bought: false },
  { id: 'yellow', color: 'yellow', price: 2000, bought: false }
];

let equippedBall = 'white';

function loadBallStore() {
  const saved = localStorage.getItem('ballStore');
  if (saved) {
    const data = JSON.parse(saved);
    ballStore = ballStore.map(b => {
      const s = data.find(x => x.id === b.id);
      return s ? { ...b, bought: s.bought } : b;
    });
  }

  equippedBall = localStorage.getItem('equippedBall');
  if (!ballStore.some(b => b.id === equippedBall)) equippedBall = 'white';

  document.getElementById('bigBall').style.background = ballStore.find(b => b.id === equippedBall).color;
}

function saveBallStore() {
  localStorage.setItem('ballStore', JSON.stringify(ballStore));
  localStorage.setItem('equippedBall', equippedBall);
}

function renderBallStore() {
  const container = document.getElementById('colorStoreList');
  container.innerHTML = '';

  ballStore.forEach(b => {
    const card = document.createElement('div');
    card.className = 'color-card';
    if (equippedBall === b.id) card.classList.add('equipped');

    card.innerHTML = `
      <div class="ballPreview" style="background:${b.color}"></div>
      <h3>Color: ${b.color}</h3>
      <p>Precio: ${b.price}</p>
      <button>${!b.bought ? 'Comprar' : (equippedBall === b.id ? 'Desequipar' : 'Equipar')}</button>
    `;
    container.appendChild(card);

    const btn = card.querySelector('button');
    btn.disabled = !b.bought && score < b.price;

    btn.addEventListener('click', () => {
      if (!b.bought && score >= b.price) {
        score -= b.price;
        b.bought = true;
        showToast(`💎 Compraste la bola ${b.color}`);
      } else if (b.bought) {
        equippedBall = (equippedBall === b.id) ? 'white' : b.id;
      }
      document.getElementById('bigBall').style.background = ballStore.find(x => x.id === equippedBall).color;
      saveBallStore();
      renderBallStore();
      updateUI();
    });
...
document.getElementById('storeBtn').addEventListener('click', () => {
  const menu = document.getElementById('colorStoreMenu');
  menu.classList.remove('hidden');
  renderBallStore(); 
});
