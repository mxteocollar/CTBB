...
// Variables nuevas para logros y estados
let totalClicks = 0;
let totalUpgrades = 0;
...
bigBall.addEventListener('click', () => {
  ...
  totalClicks++;                   // +1 total clicks
  recentClicks.push(Date.now());  // guardo tiempo del click para logro de velocidad
  lastClickTime = Date.now();     // actualizar tiempo del último click
  isIdle = false;                 // si clickeo, no estoy idle ni afk
  isAFK = false;
...
// Eventos para mostrar y cerrar logros
...
// Timer para detectar IDLE y AFK e ir chequeando logros de tiempo e inactividad
setInterval(() => {
  const now = Date.now();

  // Si no clickeás en 5 minutos => IDLE
  if (!isIdle && now - lastClickTime > 300000) {
    isIdle = true;
    idleStartTime = now;
  }

  // Si no tocás nada (ni clics ni mejoras) por 10 min => AFK
...
// Al cargar la página: restaurar logros y actualizar visualmente
...
// Aplica efecto al cargar y al comprar
...
// Llama a saveAchievements, save permanent y demás
function saveGame() {
  saveAchievements();
  savePermanent();
  // Podés guardar más aquí (score, levels…) si implementás luego
}

// Ajusta clicks y pps con buffs antes de sumar
bigBall.replaceWith( (() => {
...
// Modifica setInterval de auto-clicks
clearInterval(); // limpia el anterior si usaste var global
setInterval(() => {
  if (clicksPerSecond > 0) {
...
// Eventos para abrir/cerrar menú permanente
